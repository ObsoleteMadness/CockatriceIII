...
uae_u32 REGPARAM2 op_emulop_1(uae_u32 opcode)
{
	opcode = opcode_swap(opcode);
...
uae_u32 REGPARAM2 op_emulop_return_1(uae_u32 opcode)
{
	opcode = opcode_swap(opcode);
	(void)opcode;
	cockatrice_m68k_emulop_return();
	m68k_incpc_normal(2);
	return 4;
}
void REGPARAM2 op_emulop_return_1_noret(uae_u32 opcode)
{
	op_emulop_return_1(opcode);
}
...
static void build_cpufunctbl ()
{
	...
	/* Basilisk EmulOps (0x7100..0x713F): dedicated handlers like macemu table68k
	 * EMULOP / EMULOP_RETURN — not op_illg. Must run before JIT cpufunctbl swap. */
	for (opcode = 0x7100; opcode <= 0x713f; opcode++) {
		if (opcode == 0x7100) {
			cpufunctbl[opcode] = op_emulop_return_1;
			cpufunctbl_noret[opcode] = op_emulop_return_1_noret;
		} else {
			cpufunctbl[opcode] = op_emulop_1;
			cpufunctbl_noret[opcode] = op_emulop_1_noret;
		}
	}
...
/*
 * Completes CPU table and memory-accessor wiring that Amiberry normally
 * does inside m68k_go() (which Cockatrice does not call: we loop m68k_run()).
 *
 * prefs_changed_cpu() copies currprefs into the cycle-timing fields.
 * build_cpufunctbl() fills cpufunctbl[] from cpuemu_0/40/50 and, with JIT,
 * calls build_comp(). set_x_funcs() assigns x_get_iword and friends; leaving
 * those NULL is a tail-call to address 0 on the first execute_normal().
 */
void m68k_prepare(void)
{
	prefs_changed_cpu();
	build_cpufunctbl();
	set_x_funcs();
...
static void Exception_mmu (int nr, uaecptr oldpc)
{
	...
	if (nr == 2) { // bus error
		...
		printf("[UAE] Address Error fault=0x%08X PC=0x%08X A7=0x%08X SR=0x%04X\n",
			(unsigned)last_fault_for_exception_3, (unsigned)currpc,
			(unsigned)m68k_areg(regs, 7), (unsigned)(regs.sr & 0xffff));
		fflush(stdout);
#ifdef DEBUGGER
...
/* Cockatrice III EmulOp / EXEC_RETURN hook (0x7100..0x713F). */
extern "C" int cockatrice_uae_illg(uae_u32 opcode);
extern "C" void cockatrice_uae_fline_trap(uae_u32 opcode, uaecptr pc, int from_mmu_op);

/*
 * Returns the PC after a 68040 MMU coprocessor (0xF0xx) op when Basilisk has
 * no MMU. MMUOP030 always has a 16-bit extension word; EA modes may add more.
 */
static uaecptr cockatrice_skip_mmu030_opcode(uaecptr pc, uae_u32 opcode)
{
	uaecptr newpc = pc + 2;
	int mode = (opcode >> 3) & 7;
	int reg = opcode & 7;
	...
	if (mode == 6) {
		...
		switch (reg) {
		case 0: newpc += 2; break;
		case 1: newpc += 4; break;
		case 2:
		case 3: newpc += 2; break;
		case 4: newpc += 2; break;
		case 5: newpc += 4; break;
		default: break;
		}
	}
	return newpc;
...
uae_u32 REGPARAM2 op_illg (uae_u32 opcode)
{
	...
	/* Basilisk II patched ROM uses MOVEQ-form opcodes 0x71xx as host traps. */
	if (cockatrice_uae_illg(opcode)) {
		m68k_incpc_normal(2);
		fill_prefetch();
		return 4;
	}
	...
	/* Basilisk II: no MMU — 68040 MMU coprocessor (0xF0xx) opcodes in Mac code are
	 * NOPs with a 2-byte advance. Do not treat all 0xFxxx as NOP: heap fill uses
	 * mixed patterns and ROM headers (e.g. 0xF1AC at 40800000) are data, not code. */
	if (!currprefs.mmu_model && (opcode & 0xFF00) == 0xF000) {
		cockatrice_uae_fline_trap(opcode, pc, 0);
		m68k_incpc_normal(2);
		fill_prefetch();
		return 4;
	}
...
void mmu_op (uae_u32 opcode, uae_u32 extra)
{
	...
	if ((opcode & 0xFE0) == 0x0500) {
		...
		/* PLPA — Basilisk has no MMU; 68040 System/extension code must not trap */
		if (currprefs.cpu_model >= 68040 && currprefs.cpu_model != 68060) {
			return;
		}
		if (currprefs.cpu_model == 68060) {
			int regno = opcode & 7;
...
#endif
	/* No MMU: stray cache/MMU F-line ops are NOPs, not Mac type-10 Line-1111 traps */
	if ((opcode & 0xF000) == 0xF000) {
#ifdef USE_JIT
		if (flush_icache)
			flush_icache(0);
	...
	cockatrice_uae_fline_trap(opcode, m68k_getpc(), 1);
...
#endif
		/* macemu: BRK ends nested m68k_execute (Execute68k / Execute68kTrap) only.
		 * Top-level Amiberry boot also uses BRK; do not exit m68k_run_jit then. */
...
void cpu_halt(int id)
{
	...
	if (!regs.halted) {
		...
		if (id == CPU_HALT_DOUBLE_FAULT) {
			printf("[UAE] Double fault (exception during bus/address fault) PC=0x%08X SP=0x%08X\n",
				(unsigned)M68K_GETPC, (unsigned)m68k_areg(regs, 7));
			fflush(stdout);
		}
...
/*
 * Runs one interpreter timeslice (exits on SPCFLAG_MODE_CHANGE). Never enters
 * m68k_run_jit. Basilisk Execute68k / Execute68kTrap call this from EmulOp
 * handlers so nested host calls do not re-enter the JIT pushall trampoline.
 */
void m68k_run_interpreter_slice(void)
{
	void (*run_func)(void);
