...
static uae_u32 mmu_fill_atc(uaecptr addr, bool super, uae_u32 tag, bool write, struct mmu_atc_line *l, uae_u32 *status060)
{
    ...
    TRY(prb) {
		...
		mddm.descriptor[2] = desc_addr;
		desc = desc_get_long(desc_addr);
		if ((desc & 3) == 2) {
            /* indirect */
            desc_addr = desc & MMU_PAGE_INDIRECT_MASK;
			mddm.descriptor[3] = desc_addr;
			desc = desc_get_long(desc_addr);
		}
		if ((desc & 1) == 1) {
            wp |= desc;
...
uaecptr debug_mmu_translate(uaecptr addr, uae_u32 val, bool super, bool data, bool write, int size, struct mmu_debug_data **mdd)
{
	...
	*mdd = &mddm;
	...
	if (!data) {
		...
		if (res == TTR_OK_MATCH) {
			mddm.tt = 1;
			mddm.ttdata = regs.itt0;
			return addr;
		}
		res = mmu_do_match_ttr(regs.itt1, addr, super);
		if (res == TTR_OK_MATCH) {
			mddm.tt = 2;
			mddm.ttdata = regs.itt1;
			return addr;
		}
	} else {
		...
		if (res == TTR_OK_MATCH) {
			mddm.tt = 1;
			mddm.ttdata = regs.dtt0;
			return addr;
		}
		res = mmu_do_match_ttr(regs.dtt1, addr, super);
		if (res == TTR_OK_MATCH) {
			mddm.tt = 2;
			mddm.ttdata = regs.dtt1;
			return addr;
		}
	}
...
{
	/*
	 * Basilisk II has no 680x0 MMU. Mac System 7/8 code (BlockMove, extensions)
	 * still executes PFLUSH/PTEST/PLPA F-line ops; without handlers they become
	 * Line 1111 traps (Mac System Error type 10).
	 */
	if ((opcode & 0xFE0) == 0x0500) {
		...
		/* PTEST — NOP */
	} else if ((opcode & 0xFFB8) == 0xF588) {
		/* PLPA — NOP */
	} else if ((opcode & 0xF000) == 0xF000) {
		/* Other 68040 MMU F-line opcodes — flush caches, do not trap */
		flush_internals();
	} else {
		op_illg(opcode);
	}
