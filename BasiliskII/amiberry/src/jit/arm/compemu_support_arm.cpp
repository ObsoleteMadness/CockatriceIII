...
#ifdef UAE
    /* Cockatrice Basilisk II EmulOps (0x7100..0x713F): end JIT blocks here so
     * prefetch/PC stay in sync across host trap dispatch (see op_illg). */
...
void flush_icache_hard(int n)
{
    ...
    current_compile_p = compiled_code;
    /* macemu uses SPCFLAG_JIT_EXEC_RETURN; MODE_CHANGE exits m68k_run_jit. */
