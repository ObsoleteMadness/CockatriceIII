...
/* Direct pc_p access on flat NATMEM; when canbang is false (Cockatrice banked Mac
 * RAM) extension-word reads must use get_*i(m68k_getpc()) so unmapped holes
 * become 680x0 bus errors instead of host SIGSEGV. */

STATIC_INLINE uae_u32 get_dibyte(int o)
{
	if (!canbang)
		return get_byte(m68k_getpc() + (uae_u32)o + 1);
	return do_get_mem_byte((uae_u8 *)((regs).pc_p + (o) + 1));
}
STATIC_INLINE uae_u32 get_diword(int o)
{
	if (!canbang)
		return get_wordi(m68k_getpc() + (uae_u32)o);
	return do_get_mem_word((uae_u16 *)((regs).pc_p + (o)));
}
STATIC_INLINE uae_u32 get_dilong(int o)
{
	if (!canbang)
		return get_longi(m68k_getpc() + (uae_u32)o);
...
extern void m68k_prepare(void);
/* Interpreter-only timeslice (no JIT); used for nested Execute68k from EmulOp. */
extern void m68k_run_interpreter_slice(void);
