...
uint32_t EMIT_SUB_ext(struct TranslatorContext *ctx, uint16_t opcode);

uint32_t EMIT_SUB_mem(struct TranslatorContext *ctx, uint16_t opcode);

uint32_t EMIT_SUB_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    ...
    if (direction == 0)
    {
        ...
        switch(size)
        {
            case 4:
                if (update_mask == 0)
                    EMIT(ctx, sub_reg(dest, dest, src, LSL, 0));
                else
                    EMIT(ctx, subs_reg(dest, dest, src, LSL, 0));
                break;
            case 2:
                if (update_mask == 0 || update_mask == SR_Z || update_mask == SR_N) {
                    tmp = RA_AllocARMRegister(ctx);
                    EMIT(ctx, 
                        sub_reg(tmp, dest, src, LSL, 0),
                        bfxil(dest, tmp, 0, 16)
                    );
    ...
    else
    {
        ...
        case 4:
            ...
            /* Perform calcualtion */
            if (update_mask == 0)
                EMIT(ctx, sub_reg(tmp, tmp, src, LSL, 0));
            else
                EMIT(ctx, subs_reg(tmp, tmp, src, LSL, 0));
...
uint32_t EMIT_SUBA_ext(struct TranslatorContext *ctx, uint16_t opcode);

uint32_t EMIT_SUBA_mem(struct TranslatorContext *ctx, uint16_t opcode);

uint32_t EMIT_SUBA_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t ext_words = 0;
    ...
    else
    {
        ...
        if (immed)
        {
            offset = ((int16_t)cache_read_16(ICACHE, (uintptr_t)&ctx->tc_M68kCodePtr[0]) << 16) | (uint16_t)cache_read_16(ICACHE, (uintptr_t)&ctx->tc_M68kCodePtr[1]);
            ...
            {
                EMIT(ctx, sub_immed_lsl12(reg, reg, offset >> 12));
                ext_words = 2;
            }
            else
            {
    ...
    if (immed == 0)
        EMIT(ctx, sub_reg(reg, reg, tmp, LSL, 0));
...
uint32_t EMIT_SUBX_ext(struct TranslatorContext *ctx, uint16_t opcode);

uint32_t EMIT_SUBX_mem(struct TranslatorContext *ctx, uint16_t opcode);

uint32_t EMIT_SUBX_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
        ...
        switch (size)
        {
            case 0: /* Byte */
                ...
                if (update_mask & SR_XVC) {
                    ...
                    EMIT(ctx, 
                        eor_reg(tmp_3, tmp_2, regx, LSL, 0), // D ^ S -> tmp_3
                        eor_reg(tmp_2, tmp_2, tmp, LSL, 0),  // D ^ R -> tmp_2
                        and_reg(tmp_3, tmp_2, tmp_3, LSL, 0),// V = (D^S) & (D^R), bit 7
            ...
            case 1: /* Word */
                ...
                if (update_mask & SR_XVC) {
                    ...
                    EMIT(ctx, 
                        eor_reg(tmp_3, tmp_2, regx, LSL, 0), // D ^ S -> tmp_3
                        eor_reg(tmp_2, tmp_2, tmp, LSL, 0),  // D ^ R -> tmp_2
                        and_reg(tmp_3, tmp_2, tmp_3, LSL, 0), // V = (D^S) & (D^R), bit 15
        ...
        switch (size)
        {
            case 0: /* Byte */
                ...
                if (update_mask & SR_XVC) {
                    ...
                    EMIT(ctx, 
                        eor_reg(tmp_3, dest, src, LSL, 0), // D ^ S -> tmp_3
                        eor_reg(tmp_2, dest, tmp, LSL, 0),  // D ^ R -> tmp_2
                        and_reg(tmp_3, tmp_2, tmp_3, LSL, 0), // V = (D^S) & (D^R), bit 7
                        mov_reg(tmp_2, tmp),                // C at position 8
                        bfi(tmp_2, tmp_3, 0, 8),            // C at position 8, V at position 7
                        bfxil(cc, tmp_2, 7, 2)
                    );
...
uint32_t EMIT_SUB_ext(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_SUB_reg(ctx, opcode);
}
...
uint32_t EMIT_SUBA_ext(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_SUBA_reg(ctx, opcode);
}
...
uint32_t EMIT_SUBX_ext(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_SUBX_reg(ctx, opcode);
}
