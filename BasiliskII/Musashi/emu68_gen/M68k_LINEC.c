...
uint32_t EMIT_AND(struct TranslatorContext *ctx, uint16_t opcode);

static uint32_t EMIT_AND_reg(struct TranslatorContext *ctx, uint16_t opcode);

static uint32_t EMIT_AND_mem(struct TranslatorContext *ctx, uint16_t opcode);
...
uint32_t EMIT_AND(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_AND_reg(ctx, opcode);
}

static uint32_t EMIT_AND_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_AND_mem(ctx, opcode);
}

static uint32_t EMIT_AND_mem(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_AND_ext(ctx, opcode);
}
