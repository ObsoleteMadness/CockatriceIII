...
static uint32_t EMIT_ASR_mem(struct TranslatorContext *ctx, uint16_t opcode);
...
static uint32_t EMIT_LSR_mem(struct TranslatorContext *ctx, uint16_t opcode);
...
static uint32_t EMIT_ROXR_mem(struct TranslatorContext *ctx, uint16_t opcode);
...
static uint32_t EMIT_ROR_mem(struct TranslatorContext *ctx, uint16_t opcode);
...
uint32_t EMIT_ASL_L_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
    uint8_t shift_reg = RA_MapM68kRegister(ctx, (opcode >> 9) & 7);
    ...
    if ((update_mask & SR_XVC) == 0) {
        ...
        */
        ...
        /*  
            C is cleared for shift 0, V is cleared if MSB not changed
            so it is safe to clear them right now 
        */
        ...
        /*
            If shift by zero, jump to this unusual condition -
            that will be simply at the end of the code, but before
            eventual NZ testing
        */
        EMIT(ctx, 
            tst_immed(shift_reg, 6, 0),
            b_cc(A64_CC_EQ, 0)
        );

        /* Remember where to patch for zero shift branch*/
        branch_if_zero_shift = ctx->tc_CodePtr - 1;

        /* 
            If X flag is used, clear it now. Cannot combine that with VC above because for zero 
            shift X remains unaffected
        */
        if (update_mask & SR_X) {
            EMIT_ClearFlags(ctx, cc, SR_CCR);
        }

        /* If V flag will be needed, get source register and shift it left */
        if (update_mask & SR_V) {
            EMIT(ctx, lsl64(tmp, reg, 32));
        }
        ...
        /* If X or C needs to be set, extract the last bit shifted out */
        if (update_mask & SR_XC) {
            uint8_t alt_flags = update_mask & SR_XC;
            if ((alt_flags & 3) != 0 && (alt_flags & 3) < 3)
            alt_flags ^= 3;

            EMIT(ctx, tst64_immed(result, 1, 32, 1));
            EMIT_SetFlagsConditional(ctx, cc, alt_flags, A64_CC_NE);
        }

        /* Now the part different for LSL and ASL: In case of ASL set V flag if the MSB changed at any time */
        if (update_mask & SR_V) {
            mask = RA_AllocARMRegister(ctx);
            EMIT(ctx,
                // Shift right to repeat MSB up to shift_reg times
                asrv64(mask, tmp, shift_reg),
                // EOR it with the shifted register, to get changed bits
                eor64_reg(mask, tmp, mask, LSL, 0),
                // Create test mask
                mov64_immed_u16(tmp, 0x8000, 3),
                asrv64(tmp, tmp, shift_reg),
                // Now test only bits described in the mask,
                tst64_reg(tmp, mask, LSL, 0),
            );
            EMIT_SetFlagsConditional(ctx, cc, SR_Valt, A64_CC_NE);
        }

        /* We are almost there, branch on zero shift jumps here */
    ...
    RA_FreeARMRegister(ctx, masked_shift);
...
uint32_t EMIT_ASL_W_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
    ...
    if ((update_mask & SR_XVC) == 0) {
        ...
        */
        ...
        if (update_mask & SR_XC) {
            ...
            EMIT(ctx, tst_immed(result, 1, 16));
            EMIT_SetFlagsConditional(ctx, cc, alt_flags, A64_CC_NE);
        }
...
uint32_t EMIT_ASL_B_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
    ...
    if ((update_mask & SR_XVC) == 0) {
        ...
        */
        ...
        if (update_mask & SR_XC) {
            ...
            EMIT(ctx, tst_immed(result, 1, 24));
            EMIT_SetFlagsConditional(ctx, cc, alt_flags, A64_CC_NE);
        }
...
uint32_t EMIT_ASR_L_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
    ...
    if ((update_mask & SR_XVC) == 0) {
        ...
        EMIT(ctx,
            sxtw64(result, reg),
            asrv64(result, result, shift_reg),
            mov_reg(reg, result)
        );
        ...
        /*  
            C is cleared for shift 0, V is cleared if MSB not changed
            in case of ASR (right), V never changes, so clearing it is fine here
        */
        if (update_mask & SR_VC) {
            EMIT_ClearFlags(ctx, cc, SR_VC);
        }
        ...
        */
        ...
        /* Sign extend register so we can safely do 64-bit ASR */
        EMIT(ctx, sxtw64(reg, reg));

        /* If X or C is necessary, rotate right into temporary just like the shift would do  */
        if (update_mask & SR_XC) {
            EMIT(ctx, rorv64(tmp, reg, shift_reg));
        }

        /* Now do the shift and truncate result back to 32 bit */
        EMIT(ctx,
            asrv64(result, reg, shift_reg),
            mov_reg(reg, result)
        );
        ...
        if (update_mask & SR_XC) {
            ...
            EMIT(ctx, tst64_immed(tmp, 1, 1, 1));
            EMIT_SetFlagsConditional(ctx, cc, alt_flags, A64_CC_NE);
        }
...
uint32_t EMIT_ASR_W_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
    ...
    if ((update_mask & SR_XVC) == 0) {
        ...
        */
        ...
        /* Sign extend register so we can safely do 64-bit ASR */
        EMIT(ctx, sxth64(result, reg));

        /* If X or C is necessary, rotate right into temporary just like the shift would do  */
        if (update_mask & SR_XC) {
            EMIT(ctx, rorv64(tmp, result, shift_reg));
        }
...
uint32_t EMIT_ASR_B_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
...
static uint32_t EMIT_ASR(struct TranslatorContext *ctx, uint16_t opcode);
...
uint32_t EMIT_LSL_L_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
...
uint32_t EMIT_LSL_W_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
...
uint32_t EMIT_LSL_B_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
...
uint32_t EMIT_LSR_L_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
...
uint32_t EMIT_LSR_W_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
    ...
    if ((update_mask & SR_XVC) == 0) {
        ...
        EMIT(ctx,
            uxth64(result, reg),
            lsrv64(result, result, shift_reg),
            bfi(reg, result, 0, 16)
        );
...
uint32_t EMIT_LSR_B_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    uint8_t cc = 0xff;
    uint8_t update_mask = M68K_GetSRMask(ctx->tc_M68kCodePtr - 1);
    uint8_t reg = RA_MapM68kRegister(ctx, opcode & 7);
...
static uint32_t EMIT_ROR_reg(struct TranslatorContext *ctx, uint16_t opcode);

static uint32_t EMIT_ROL_reg(struct TranslatorContext *ctx, uint16_t opcode);

static uint32_t EMIT_ROR(struct TranslatorContext *ctx, uint16_t opcode);
...
static uint32_t EMIT_ROXR_reg(struct TranslatorContext *ctx, uint16_t opcode);

static uint32_t EMIT_ROXL_reg(struct TranslatorContext *ctx, uint16_t opcode);

static uint32_t EMIT_ROXR(struct TranslatorContext *ctx, uint16_t opcode);
...
static struct OpcodeDef InsnTable[4096] = {
	...
	[00050 ... 00057] = { EMIT_LSR_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[00150 ... 00157] = { EMIT_LSR_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[00250 ... 00257] = { EMIT_LSR_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[01050 ... 01057] = { EMIT_LSR_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[01150 ... 01157] = { EMIT_LSR_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[01250 ... 01257] = { EMIT_LSR_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[02050 ... 02057] = { EMIT_LSR_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[02150 ... 02157] = { EMIT_LSR_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[02250 ... 02257] = { EMIT_LSR_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[03050 ... 03057] = { EMIT_LSR_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[03150 ... 03157] = { EMIT_LSR_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[03250 ... 03257] = { EMIT_LSR_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[04050 ... 04057] = { EMIT_LSR_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[04150 ... 04157] = { EMIT_LSR_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[04250 ... 04257] = { EMIT_LSR_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[05050 ... 05057] = { EMIT_LSR_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[05150 ... 05157] = { EMIT_LSR_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[05250 ... 05257] = { EMIT_LSR_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[06050 ... 06057] = { EMIT_LSR_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[06150 ... 06157] = { EMIT_LSR_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[06250 ... 06257] = { EMIT_LSR_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[07050 ... 07057] = { EMIT_LSR_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[07150 ... 07157] = { EMIT_LSR_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[07250 ... 07257] = { EMIT_LSR_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[00450 ... 00457] = { EMIT_LSL_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[00550 ... 00557] = { EMIT_LSL_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[00650 ... 00657] = { EMIT_LSL_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[01450 ... 01457] = { EMIT_LSL_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[01550 ... 01557] = { EMIT_LSL_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[01650 ... 01657] = { EMIT_LSL_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[02450 ... 02457] = { EMIT_LSL_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[02550 ... 02557] = { EMIT_LSL_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[02650 ... 02657] = { EMIT_LSL_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[03450 ... 03457] = { EMIT_LSL_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[03550 ... 03557] = { EMIT_LSL_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[03650 ... 03657] = { EMIT_LSL_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[04450 ... 04457] = { EMIT_LSL_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[04550 ... 04557] = { EMIT_LSL_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[04650 ... 04657] = { EMIT_LSL_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[05450 ... 05457] = { EMIT_LSL_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[05550 ... 05557] = { EMIT_LSL_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[05650 ... 05657] = { EMIT_LSL_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[06450 ... 06457] = { EMIT_LSL_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[06550 ... 06557] = { EMIT_LSL_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[06650 ... 06657] = { EMIT_LSL_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
	...
	[07450 ... 07457] = { EMIT_LSL_B_reg, NULL, SR_X, SR_CCR, 1, 0, 1 },
	...
	[07550 ... 07557] = { EMIT_LSL_W_reg, NULL, SR_X, SR_CCR, 1, 0, 2 },
	...
	[07650 ... 07657] = { EMIT_LSL_L_reg, NULL, SR_X, SR_CCR, 1, 0, 4 },
...
static uint32_t EMIT_ASR_mem(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_ASL_mem(ctx, opcode);
}

static uint32_t EMIT_LSR_mem(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_LSL_mem(ctx, opcode);
}

static uint32_t EMIT_ROXR_mem(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_ROXL_mem(ctx, opcode);
}

static uint32_t EMIT_ROR_mem(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_ROL_mem(ctx, opcode);
}

static uint32_t EMIT_ASR(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_ASL(ctx, opcode);
}

static uint32_t EMIT_ROR_reg(struct TranslatorContext *ctx, uint16_t opcode)
{
    return EMIT_ROL(ctx, opcode);
}
