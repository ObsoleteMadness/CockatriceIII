...
static void fpgen_rm_reg(uint16 w2)
{
	...
	switch (opmode)
	{
		...
		case 0x0b:		// FTWOTOXM1
		{
			REG_FP[dst] = double_to_fx80(exp2(fx80_to_double(source)) - 1.0);
			SET_CONDITION_CODES(REG_FP[dst]);
			USE_CYCLES(400);
			break;
		}
		...
		case 0x1f:		// FGETMAN
		{
			double d = fx80_to_double(source);
			int exp;
			double mant = frexp(d, &exp) * 2.0;
			REG_FP[dst] = double_to_fx80(mant);
			SET_CONDITION_CODES(REG_FP[dst]);
			USE_CYCLES(6);
			break;
		}
		...
		case 0x1e:		// FGETEXP
		{
			...
			int exp;
			frexp(d, &exp);
			REG_FP[dst] = double_to_fx80((double)(exp - 1));
			SET_CONDITION_CODES(REG_FP[dst]);
			USE_CYCLES(6);
			break;
		}
		...
		case 0x37:		// FSINCOS
		{
			double rad = fx80_to_double(source);
			int cos_reg = opmode & 7;
			REG_FP[cos_reg] = double_to_fx80(cos(rad));
			REG_FP[dst] = double_to_fx80(sin(rad));
			SET_CONDITION_CODES(REG_FP[dst]);
			USE_CYCLES(400);
			break;
		}
		case 0x26:		// FSCALE
		{
			double src_d = fx80_to_double(source);
			double dst_d = fx80_to_double(REG_FP[dst]);
			REG_FP[dst] = double_to_fx80(ldexp(dst_d, (int)src_d));
			SET_CONDITION_CODES(REG_FP[dst]);
			USE_CYCLES(30);
			break;
		}
...
static void fmove_fpcr(uint16 w2)
{
	...
	if (dir)	// From system control reg to <ea>
	{
		if (mode == 0) // Dn
		{
			if (reg & 4) REG_D[reg_num] = REG_FPCR;
			else if (reg & 2) REG_D[reg_num] = REG_FPSR;
			else if (reg & 1) REG_D[reg_num] = REG_FPIAR;
		}
		else if (mode == 4) // -(An)
		{
			if (reg & 1) { uint32 a = EA_AY_PD_32(); m68ki_write_32(a, REG_FPIAR); }
			if (reg & 2) { uint32 a = EA_AY_PD_32(); m68ki_write_32(a, REG_FPSR); }
			if (reg & 4) { uint32 a = EA_AY_PD_32(); m68ki_write_32(a, REG_FPCR); }
		}
		else
		{
			uint32 a = 0;
			switch (mode)
			{
				case 2: a = REG_A[reg_num]; break;
				case 3: a = EA_AY_PI_32(); break;
				case 5: a = EA_AY_DI_32(); break;
				case 6: a = EA_AY_IX_32(); break;
				case 7:
					switch (reg_num)
					{
						case 0: a = EA_AW_32(); break;
						case 1: a = EA_AL_32(); break;
						default: a = 0; break;
					}
					break;
				default: a = 0; break;
			...
			if (mode == 3)
			{
				if (reg & 4) { m68ki_write_32(a, REG_FPCR); a += 4; REG_A[reg_num] += 4; }
				if (reg & 2) { m68ki_write_32(a, REG_FPSR); a += 4; REG_A[reg_num] += 4; }
				if (reg & 1) { m68ki_write_32(a, REG_FPIAR); a += 4; REG_A[reg_num] += 4; }
			}
			else if (a)
			{
				if (reg & 4) { m68ki_write_32(a, REG_FPCR); a += 4; }
				if (reg & 2) { m68ki_write_32(a, REG_FPSR); a += 4; }
				if (reg & 1) { m68ki_write_32(a, REG_FPIAR); a += 4; }
			}
		}
	}
	else		// From <ea> to system control reg
	{
		if (mode == 0) // Dn
		{
			if (reg & 4) { REG_FPCR = REG_D[reg_num]; float_rounding_mode = (REG_FPCR >> 4) & 0x3; }
			if (reg & 2) REG_FPSR = REG_D[reg_num];
			if (reg & 1) REG_FPIAR = REG_D[reg_num];
		}
		else if (mode == 3) // (An)+
		{
			if (reg & 4) { uint32 a = EA_AY_PI_32(); REG_FPCR = m68ki_read_32(a); float_rounding_mode = (REG_FPCR >> 4) & 0x3; }
			if (reg & 2) { uint32 a = EA_AY_PI_32(); REG_FPSR = m68ki_read_32(a); }
			if (reg & 1) { uint32 a = EA_AY_PI_32(); REG_FPIAR = m68ki_read_32(a); }
		}
		else
		{
			uint32 a = 0;
			switch (mode)
			{
				...
				case 7:
					switch (reg_num)
					{
						case 0: a = EA_AW_32(); break;
						case 1: a = EA_AL_32(); break;
						case 2: a = EA_PCDI_32(); break;
						case 3: a = EA_PCIX_32(); break;
						case 4: a = OPER_I_32(); break;
						default: a = 0; break;
					}
					break;
				default: a = 0; break;
			...
			if (mode == 7 && reg_num == 4)
			{
				if (reg & 4) { REG_FPCR = a; float_rounding_mode = (REG_FPCR >> 4) & 0x3; }
				if (reg & 2) REG_FPSR = a;
				if (reg & 1) REG_FPIAR = a;
			}
			else if (a)
			{
				if (reg & 4) { REG_FPCR = m68ki_read_32(a); float_rounding_mode = (REG_FPCR >> 4) & 0x3; a += 4; }
				if (reg & 2) { REG_FPSR = m68ki_read_32(a); a += 4; }
				if (reg & 1) { REG_FPIAR = m68ki_read_32(a); a += 4; }
			}
		}
	}
...
static void fmovem(uint16 w2)
{
	...
	if (dir)	// From FP regs to mem
	{
		...
		else
		{
			...
			for (i = 0; i < 8; i++)
			{
				...
				{
					if (imode == 3) {
						WRITE_EA_FPE(imode, reg, REG_FP[i], 0);
					} else {
	...
	else		// From mem to FP regs
	{
		...
		for (i = 0; i < 8; i++)
		{
			...
			{
				if (imode == 3) {
					REG_FP[i] = READ_EA_FPE(imode, reg, 0);
				} else {
...
static void fdbcc_fscc_ftrapcc(void)
{
	...
	{
		if (reg == 3) OPER_I_16(); // 16-bit operand
		else if (reg == 4) OPER_I_32(); // 32-bit operand
		if (TEST_CONDITION(condition))
		{
